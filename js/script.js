...
document.querySelectorAll('button').forEach(item => { //Checks when a button is pressed so that it can change the page
   item.addEventListener('click', (e) => {
     ...
     for(x = 0; x < document.querySelectorAll(".page").length; x++) {
      ...
      document.querySelectorAll('button')[parseInt(e.target.innerHTML) * document.querySelectorAll(".page").length - document.querySelectorAll(".page").length + (parseInt(e.target.innerHTML) - 1)].className = "active"
     }
